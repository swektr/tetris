...
fn main() {
    ...
    let mut curr_piece = gen_piece();
    let mut next_piece = gen_piece();

    // Piece position and rotation
    let mut rot: usize = 0;
    let mut y : i32 = 0;
    let mut x = 10/2 - curr_piece.state(rot).width() as i32 / 2;
    let mut dx: i32 = 0;
    ...
    loop {
        gfx::draw_next(next_window, next_piece.state(0));
        gfx::draw_board(game_window, &board);
        gfx::draw_score_win(score_window, &gs);
        if dx != 0 && ! board.check_collision(curr_piece.state(rot), y, x+dx){
            x = x+dx;
        }
        ...
        if n_ticks >= tick_threshold {
            n_ticks = 0;
            if ! board.check_collision(curr_piece.state(rot), y+1, x){
                y += 1;
            } else {
                board.place_piece(curr_piece.state(rot), y as usize, x as usize);
                curr_piece = next_piece;
                next_piece = gen_piece();
                ...
                rot = 0;
                y   = 0;
                x   = 10/2 - curr_piece.state(rot).width() as i32 / 2;
                speed_mult = 1;
                // Check for overflow
                if board.check_collision(curr_piece.state(rot), y, x) {
                    break;
                }
            }
        }
        gfx::draw_piece(game_window, curr_piece.state(rot), y, x);
        gfx::doupdate();
        ...
        match ncurses::getch() {
            ...
            ncurses::KEY_UP    => {
                ...
                if ! board.check_collision(curr_piece.state(temp_rot), y, x) {
                    rot = temp_rot;
                }
            },
            ncurses::KEY_DOWN  => {
